...
def test_distribution_audit_passes_current_repo_without_company_or_project_coupling() -> None:
    ...
    assert "docs/robots.txt" in result["checks"]["required_files"]
    assert "docs/sitemap.xml" in result["checks"]["required_files"]
    assert "docs/assets/social-preview.png" in result["checks"]["required_files"]
